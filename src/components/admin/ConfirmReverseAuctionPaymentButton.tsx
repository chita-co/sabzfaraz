...
import { confirmReverseAuctionOfflinePayment, rejectReverseAuctionOfflinePayment } from "@/app/admin/reverse-auctions/payment-actions";

export default function ConfirmReverseAuctionPaymentButton({ orderId }: { orderId: string }) {
  ...
  async function handleConfirm() {
    ...
    setLoading(true);
    const result = await confirmReverseAuctionOfflinePayment(orderId);
    setLoading(false);
  ...
  async function handleReject() {
    ...
    setLoading(true);
    const result = await rejectReverseAuctionOfflinePayment(orderId, reason.trim());
    setLoading(false);
    if (result?.error) alert(result.error);
    else window.location.reload();
  }
