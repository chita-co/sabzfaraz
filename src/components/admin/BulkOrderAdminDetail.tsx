...
import {
  ...
  confirmDepositPayment, markBulkOrderCompleted, sendAdminBulkMessage, getAdminBulkMessages,
} from "@/app/admin/bulk-orders/actions";

// اصلاح: اضافه کردن productId به StoreItem برای تطابق با StoreItemInput
...
export default function BulkOrderAdminDetail({
  request, bankAccounts, receiptImageUrl,
}: { request: BulkOrderRequest; bankAccounts: BankAcc[]; receiptImageUrl: string | null }) {
  ...
  async function handleSaveNote() {
    await updateInternalNote(request.id, note);
  ...
  async function handleComplete() {
    const result = await markBulkOrderCompleted(request.id);
    if (result?.error) alert(result.error); else window.location.reload();
  }
    ...
    <div>
      ...
      <div className="admin-card mb-5">
        ...
        {marketItems.map((it, i) => (
          <div key={i} className="grid grid-cols-12 gap-2 items-center mb-2">
            <input value={it.name} onChange={(e) => updateMarketItem(i, "name", e.target.value)} className="col-span-4 admin-input" />
            <input type="number" value={it.quantity} onChange={(e) => updateMarketItem(i, "quantity", Number(e.target.value))} className="col-span-2 admin-input" />
            ...
            <button onClick={() => removeMarketItem(i)} className="col-span-2 admin-btn admin-btn-danger"><Trash2 size={13} /></button>
          </div>
        ))}
      ...
      <div className="admin-card mb-5">
        ...
        <select value={supplyChoice} onChange={(e) => setSupplyChoice(e.target.value as "REVIEWING" | "POSSIBLE" | "NOT_POSSIBLE")} className="admin-input mb-3">
          ...
          <option value="POSSIBLE">امکان‌پذیر است</option>
          <option value="NOT_POSSIBLE">امکان‌پذیر نیست</option>
        </select>
        ...
        {supplyChoice !== "REVIEWING" && (
          <button onClick={handleSaveSupplyStatus} className="admin-btn admin-btn-primary">ذخیره و اعلام به کاربر</button>
        )}
      </div>

      {receiptImageUrl && request.status === "AWAITING_PAYMENT_CONFIRMATION" && (
        <div className="admin-card mb-5">
          <h2 className="font-bold text-gray-800 mb-3">رسید پرداخت مشتری</h2>
          <Image src={receiptImageUrl} alt="رسید" width={320} height={200} style={{ borderRadius: 10, marginBottom: 12 }} />
          <div>
      ...
      {request.status === "PREPARING" && (
        <div className="admin-card mb-5">
          <button onClick={handleComplete} className="admin-btn admin-btn-primary">علامت‌گذاری به‌عنوان تکمیل‌شده / ارسال‌شده</button>
        </div>
      )}
