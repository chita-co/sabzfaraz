...
export default function TrustBadges() {
  ...
  return (
    <>
      {BADGES.map((b) => (
        <button key={b.slug} type="button" className="trust-badge-btn" onClick={() => setOpen(true)}>
      ...
      {open && (
        <div className="trust-badge-modal-overlay" onClick={() => setOpen(false)}>
          <div className="trust-badge-modal" onClick={(e) => e.stopPropagation()}>
            <button className="trust-badge-modal-close" onClick={() => setOpen(false)}><X size={18} /></button>
            {/* eslint-disable-next-line @next/next/no-img-element */}
