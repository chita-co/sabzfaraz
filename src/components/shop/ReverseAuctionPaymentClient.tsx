...
import { ShoppingBag, Clock, MapPin, Upload, Loader2, CheckCircle2 } from "lucide-react";
import { createReverseAuctionOrderOnline, createReverseAuctionOrderOffline } from "@/app/(shop)/reverse-auctions/payment-actions";

interface ReverseAuctionRow {
  id: string; title: string; images: string[]; sold_price: number | null; shipping_cost: number;
  payment_deadline: string | null; payment_status: string | null;
}
interface Address { id: string; full_name: string; phone: string; province: string; city: string; postal_code: string; address_line: string; is_default: boolean; }
...
export default function ReverseAuctionPaymentClient({
  auction, addresses, bankAccounts, alreadySubmitted, paymentResult,
}: { auction: ReverseAuctionRow; addresses: Address[]; bankAccounts: Bank[]; alreadySubmitted: boolean; paymentResult: string | null }) {
  ...
  const total = (auction.sold_price ?? 0) + (auction.shipping_cost ?? 0);
  ...
  async function handlePay() {
    ...
    setLoading(true);
    if (mode === "ONLINE") {
      await createReverseAuctionOrderOnline(auction.id, addressId);
      setLoading(false);
    } else {
      ...
      fd.append("file", file);
      const result = await createReverseAuctionOrderOffline(auction.id, addressId, method, bankAccountId, fd);
      setLoading(false);
    ...
    return (
      <div className="bg-white rounded-2xl border border-gray-200 p-8 text-center">
        ...
        <h2 className="font-bold text-gray-800 mb-2">
          {paymentResult === "success" || auction.payment_status === "PAID" ? "پرداخت شما با موفقیت ثبت شد" : "درخواست پرداخت شما ثبت شد"}
        </h2>
    ...
    <div className="space-y-5">
      <div className="loyalty-hero-card" style={{ borderColor: "#f59e0b" }}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="loyalty-tier-name" style={{ color: "#b45309" }}>🎉 شما این کالا را خریداری کردید</span>
        </div>
        <p className="text-sm text-gray-700 mb-1">{auction.title}</p>
        <div className="loyalty-balance-display" style={{ fontSize: 26 }}>{total.toLocaleString("fa-IR")} <small>تومان</small></div>
        <p className="text-xs text-gray-500 mt-1">
          (قیمت خرید: {(auction.sold_price ?? 0).toLocaleString("fa-IR")} + هزینه ارسال: {auction.shipping_cost.toLocaleString("fa-IR")})
        </p>
        {auction.payment_deadline && (
          <p className="text-xs text-red-600 flex items-center gap-1 mt-3">
            <Clock size={13} /> مهلت پرداخت تا: {new Date(auction.payment_deadline).toLocaleString("fa-IR")}
          </p>
        )}
      </div>
