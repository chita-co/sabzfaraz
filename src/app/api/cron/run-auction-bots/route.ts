...
export async function GET(request: NextRequest) {
  ...
  if (!botSettings?.enabled_global) return NextResponse.json({ status: "bots-disabled" });
  ...
  for (const auction of auctions ?? []) {
    const { data: lastBid } = await admin
      .from("auction_bids")
      .select("amount, created_at, is_bot, user_id")
    ...
    if ((botBidCount ?? 0) >= botSettings.bots_per_auction * 6) continue; // سقف تعداد کل پیشنهادهای ربات برای این مزایده
  ...
  return NextResponse.json({ status: "ok", bidsPlaced });
}