...
قوانین بسیار سخت‌گیرانه برای فیلد description (حیاتی — دقیقاً رعایت کن):
۱) خروجی باید HTML تمیز باشد؛ هیچ خط جداکننده با خط‌تیره (مثل -----)، هیچ pipe (|) و هیچ متن خارج از یک تگ HTML مجاز نیست.
۲) بین هر تگ سطح‌بالای HTML (هر <p>, <h2>, <h3>, <ul>) حتماً یک کاراکتر خط جدید (\\n) بگذار تا وقتی متن در ادیتور متنی ساده باز شود، خوانا و مرتب باشد — نه همه چسبیده به هم در یک خط.
۳) بین هر بخش اصلی (تیتر، مقدمه، مشخصات فنی، کاربردها، نکات، محتویات بسته، سوالات متداول، تبلیغ) دقیقاً یک خط کاملاً خالی بگذار (یعنی دو کاراکتر خط جدید پشت‌سرهم: \\n\\n).
۴) دقیقاً همین ساختار ۸ بخشی را رعایت کن، به همین ترتیب:

<h2>[نام کامل محصول] – [مهم‌ترین کاربرد یا ویژگی خاص آن]</h2>

<p>[پاراگراف مقدمه، ۴ تا ۵ خط: محصول را کامل معرفی کن — برند، جنس بدنه، نوع عملکرد، کاربرد کلی و مزیت اصلی. اگر محصول با چند اسم رایج در بازار شناخته می‌شود، همه‌ی این نام‌ها را طبیعی داخل متن بیاور. طوری بنویس که مشتری حس کند این محصول دقیقاً برای نیازش ساخته شده.]</p>

<h3>⚙️ مشخصات فنی</h3>
<ul>
<li>[هر ویژگی فنی در یک آیتم جدا]</li>
</ul>

<h3>📌 کاربردهای اصلی</h3>
<ul>
<li>[هر کاربرد در محیط‌های مختلف، در یک آیتم جدا]</li>
</ul>

<h3>💡 نکات مهم و نحوه استفاده صحیح</h3>
<ul>
<li>[نکات نگهداری، ایمنی و روش صحیح استفاده، هر کدام جدا]</li>
</ul>

<h3>📦 محتویات بسته</h3>
<ul>
<li>[دقیقاً چه چیزی داخل جعبه/پاکت است]</li>
</ul>

<h3>❓ سوالات متداول</h3>
<p><strong>[سؤال اول رایج مشتریان؟]</strong><br>[پاسخ کوتاه و مفید.]</p>
<p><strong>[سؤال دوم؟]</strong><br>[پاسخ.]</p>
<p><strong>[سؤال سوم؟]</strong><br>[پاسخ.]</p>

<p>[یک پاراگراف طبیعی معرفی فروشگاه سبزفراز، متناسب با دسته‌ی همین محصول، شامل sabzfaraz.ir به‌صورت متن عادی وسط جمله — بدون pipe یا علامت تزئینی.]</p>

۵) از نقطه‌گذاری کامل و صحیح فارسی (نقطه، ویرگول، دونقطه) استفاده کن. جمله‌ها روان، کامل و حرفه‌ای باشند — دقیقاً مثل بهترین فروشگاه‌های اینترنتی ایران.
۶) طول توضیحات باید کامل و غنی باشد (حداقل ۳۵۰ کلمه)، نه سطحی.
۷) حداقل ۳ سؤال در بخش سوالات متداول باشد.
...
فقط یک JSON با این ساختار دقیق برگردان (بدون Markdown fence، بدون توضیح اضافه):
{
  ...
  "description": "متن HTML دقیقاً طبق ۷ قانون بالا با \\n بین تگ‌ها و \\n\\n بین بخش‌ها",
...
export async function deletePartnerProductAction(productId: string) {
  try {
    const partner = await requireActivePartner();
    const admin = createAdminClient();
    ...
    if ((count ?? 0) > 0) {
      return { error: "این محصول در سفارش‌های قبلی مشتریان استفاده شده و برای حفظ سابقه‌ی سفارش‌ها قابل حذف کامل نیست. در صورت نیاز موجودی آن را صفر کنید تا از فروش خارج شود." };
    }

    await admin.from("product_categories").delete().eq("product_id", productId);
    await admin.from("product_quantity_tiers").delete().eq("product_id", productId);
    await admin.from("product_attributes").delete().eq("product_id", productId);
    await admin.from("partner_product_image_sources").delete().eq("product_id", productId);
    await admin.from("partner_category_suggestions").delete().eq("product_id", productId);
