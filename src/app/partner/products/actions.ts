...
۱) این متن قرار است داخل یک باکس متنی ساده (نه ویرایشگر HTML) به همکار و مشتری نمایش داده شود. به همین دلیل، مطلقاً و تحت هیچ شرایطی نباید از هیچ تگ HTML استفاده کنی — نه <p>، نه <h2>، نه <h3>، نه <ul>، نه <li>، نه <strong>، نه <br>، هیچ‌کدام. فقط و فقط متن فارسی ساده با خط جدید (\\n) بنویس.
۲) به‌جای تگ h2/h3 برای عنوان هر بخش، از یک ایموجی مناسب در ابتدای آن بخش استفاده کن (دقیقاً طبق نمونه‌ی زیر).
۳) به‌جای تگ <ul><li>، هر آیتم لیست را در یک خط جدید و با علامت «•» در ابتدای آن بنویس.
۴) بین هر بخش اصلی، دقیقاً یک خط کاملاً خالی بگذار (یعنی دو کاراکتر خط جدید پشت‌سرهم: \\n\\n).
۵) دقیقاً همین ساختار را رعایت کن، به همین ترتیب (این فقط قالب است، متن را خودت با محتوای واقعی محصول بنویس):

[نام کامل محصول] – [مهم‌ترین کاربرد یا ویژگی خاص آن]

[پاراگراف مقدمه، ۴ تا ۵ خط: محصول را کامل معرفی کن — برند، جنس بدنه، نوع عملکرد، کاربرد کلی و مزیت اصلی. اگر محصول با چند اسم رایج در بازار شناخته می‌شود، همه‌ی این نام‌ها را طبیعی داخل متن بیاور.]
...
❓ سوالات متداول
سوال: [سؤال اول رایج مشتریان؟]
پاسخ: [پاسخ کوتاه و مفید.]
...
[یک پاراگراف طبیعی معرفی فروشگاه سبزفراز، متناسب با دسته‌ی همین محصول، شامل sabzfaraz.ir به‌صورت متن عادی وسط جمله.]

۶) از نقطه‌گذاری کامل و صحیح فارسی (نقطه، ویرگول، دونقطه) استفاده کن. جمله‌ها روان، کامل و حرفه‌ای باشند.
۷) طول توضیحات باید کامل و غنی باشد (حداقل ۳۵۰ کلمه)، نه سطحی.
۸) دقیقاً ۳ تا ۴ سؤال در بخش سوالات متداول بیاور.
...
فقط یک JSON با این ساختار دقیق برگردان (بدون Markdown fence، بدون توضیح اضافه):
{
  ...
  "description": "متن ساده (بدون هیچ تگ HTML) دقیقاً طبق ۸ قانون بالا با \\n بین خطوط و \\n\\n بین بخش‌ها",
