...
export default async function NewPartnerProductPage() {
  ...
  const { data: settings } = await admin.from("partner_settings").select("frame_template_url, frame_center_x, frame_center_y, frame_center_width, frame_center_height, frame_output_size, watermark_url, watermark_enabled, watermark_opacity, watermark_rotation, watermark_scale_percent").eq("id", 1).single();
