...
export async function confirmReverseAuctionOfflinePayment(orderId: string) {
  ...
  if (!order.related_reverse_auction_id) return { error: "این سفارش به هیچ حراج معکوسی متصل نیست." };
  ...
  await createNotification(order.user_id, "پرداخت شما تأیید شد ✅", `پرداخت شما برای سفارش ${order.order_number} تأیید شد و سفارش شما به‌زودی پردازش می‌شود.`);
  const phone = (order as { address?: { phone?: string } }).address?.phone;
  if (phone) {
    try { await sendSms(phone, `سبزفراز\nپرداخت خرید شما تأیید شد.\nکد سفارش: ${order.order_number}`); }
    catch (e) { console.error("خطا در ارسال پیامک:", e); }
  }

  revalidatePath(`/admin/reverse-auctions/${order.related_reverse_auction_id}`);
  revalidatePath("/admin/orders");
  return { success: true };
}

export async function rejectReverseAuctionOfflinePayment(orderId: string, reason: string) {
  const supabase = await createClient();
  const { data: order } = await supabase.from("orders").select("user_id, order_number, related_reverse_auction_id, payment_status").eq("id", orderId).single();
  if (!order) return { error: "سفارش یافت نشد." };
  ...
  await createNotification(order.user_id, "پرداخت رد شد", `پرداخت شما برای سفارش ${order.order_number} تأیید نشد. دلیل: ${reason}. لطفاً دوباره اقدام کنید یا با پشتیبانی تماس بگیرید.`);

  if (order.related_reverse_auction_id) revalidatePath(`/admin/reverse-auctions/${order.related_reverse_auction_id}`);
  revalidatePath("/admin/orders");
  return { success: true };
}