  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-7xl px-4 py-8">
        <Breadcrumb theme="dark" items={[{ label: "محصولات استوک" }]} />

        <h1 className="mb-6 text-xl font-bold text-white">محصولات استوک</h1>
        {products && products.length > 0 ? (
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {(products as Product[]).map((p) => <ProductCard key={p.id} product={p} isWishlisted={wishlistIds.has(p.id)} />)}
          </div>
        ) : (
          <p className="text-gray-300">در حال حاضر محصولی در این بخش قرار ندارد.</p>
        )}
      </div>
    </>
  );
