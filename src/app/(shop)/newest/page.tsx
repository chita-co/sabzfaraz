...
export default async function NewestProductsPage({
  ...
  let query = supabase.from("products").select("*", { count: "exact" }).eq("is_active", true).eq("show_in_newest", true);
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-7xl px-4 py-8">
        <Breadcrumb theme="dark" items={[{ label: "جدیدترین محصولات" }]} />
        ...
        <ProductListClient mode="newest" sort={sort} initialProducts={products ?? []} initialCount={count ?? 0} initialPage={page} initialPageSize={pageSize} initialWishlistIds={wishlistIds} basePath="/newest" />
      </div>
    </>
  );
}